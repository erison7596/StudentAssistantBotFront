...
const ChatComponent = ({ chatName, setChatName, chatId, initialMessages = [], setChats }) => {
  ...
  return (
    <div className="flex flex-col max-h-screen h-full">
      <div className="flex-1 overflow-y-auto p-4 custom-height">
        {messages && messages.length > 0 ? (
          messages.map((msg, index) => (
            <div key={index} className={`mb-4 ${msg.sender === 'user' ? 'text-right' : 'text-left'}`}>
              <div className={`inline-block p-2 relative rounded-lg ${msg.sender === 'user' ? 'bg-blue-500 text-black message-user' : 'bg-gray-300 message-bot'}`}>
      ...
      <div className="flex p-4 border-t border-gray-300">
        ...
        />
        <button onClick={sendMessage} className="ml-2 p-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600">
          Enviar
        </button>
      </div>
    </div>
